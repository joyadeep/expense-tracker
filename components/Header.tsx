    ...
    <header className={` w-full sticky top-0 flex justify-between items-center border-b px-5 py-3 h-16 bg-background`}>
        ...
        <div className="flex items-center gap-5">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent>
              <DropdownMenuGroup>
                <DropdownMenuItem className="cursor-pointer text-lg" onClick={() => router.push("/profile")}> <UserRound  className="mr-2 h-5 w-5"/> Profile</DropdownMenuItem>
                {/* <DropdownMenuItem className="cursor-pointer"> <Settings className="mr-2 h-4 w-4" /> Settings</DropdownMenuItem> */}
                <DropdownMenuItem className="cursor-pointer text-lg" onClick={handleLogout}> <LogOut className="mr-2 h-5 w-5"  /> Logout</DropdownMenuItem>
              </DropdownMenuGroup>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
