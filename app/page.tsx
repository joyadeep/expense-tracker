...
export default function Home() {
  ...
  return (
    <main className="bg-background h-screen overflow-y-auto">
      <div className="w-full h-14 flex items-center justify-between px-5 md:px-16 mb-5">
       ...
       <Image alt="logo" src="/logo.svg" width={25} height={25} />
        ...
        <div className="flex gap-10 items-center">
          {
            links.map((link)=>(
              <Link className="text-foreground hover:underline hover:underline-offset-8 hidden md:block " key={link.id} href={link.link}>{link.title}</Link>
            ))
          }
     ...
     <Button variant={"primary"} size={"lg"} className="w-fit shadow-lg "  onClick={()=>router.push("/auth?type=register")}>Request a Demo</Button>
     </section>
